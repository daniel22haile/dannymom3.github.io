...
class DLinkedList {
    ...
    insertAfter(p, elem) {
        let newNode = new Node(elem, p, p._next);
        this._size++;
        return newNode;
    }
    insertBefore(p, elem) {
        let newNode = new Node(elem, p._prev, p);
        this._size++;
        return newNode;
    }
    insertFirst(elem) {
        let newNode = new Node(elem, this._header, this._header._next);
        this._size++;
        return newNode;
    }
    insertLast(elem) {
        let newNode = new Node(elem, this._trailer._prev, this._trailer);
        this._size++;
        return newNode;
    }
    remove(p) {
...
let tst0 = new DLinkedList();
tst0.print();
let tst1 = new DLinkedList();
tst1.insertFirst(5);
tst1.print();
let tst2 = new DLinkedList();
tst2.print();
tst2.insertFirst(1);
tst2.print();
tst2.insertLast(3);
